...
fun mojangRegex(c: Char = '&') = Regex("&x(?:&[0-9a-f]){6}|&[0-9a-fk-or]".replace(c, COLOR_CHAR), RegexOption.IGNORE_CASE)

fun mojangColorRegex(c: Char = '&') = Regex("&x(?:&[0-9a-f]){6}|&[0-9a-f]".replace(c, COLOR_CHAR), RegexOption.IGNORE_CASE)
 ...
 * Does not remove format codes or CMI gradient.
 */
fun String.stripColorCodes(c: Char = '&'): String = apply {
    replace(cmiColorRegex, "")
    replace(hexRegex(c), "")
    replace(bukkitRegex(c), "")
    replace(codeRegex(c), "")
    replace(mojangRegex(c), "")
}

/*
 * @return the string without any format codes.
 */
fun String.stripFormatCodes(c: Char = '&'): String = formatRegex(c).replace(this, "")

/*
 * @return converts all color codes (&0) to the mojang color code (§0).
 */
